...
class DiarizationManager:
    ...
    def _rename_speakers(self, segments: list[SpeakerSegment]) -> list[SpeakerSegment]:
        ...
        # Создаём маппинг
        speaker_map = {old_name: f"Спикер №{i+1}" for i, old_name in enumerate(speaker_order)}
...
class HybridDiarization:
    ...
    def _get_embedding_model(self):
        ...
        if self._embedding_model is None:
            try:
                ...
                raise DiarizationError(
                    "speechbrain не установлен. " "Установите: pip install speechbrain"
                )
    ...
    def diarize(
        ...
        # Создаём результат
        result = []
        for (start, end), label in zip(speech_segments, labels, strict=False):
