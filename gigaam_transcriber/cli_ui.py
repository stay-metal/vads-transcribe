...
class Metrics:
    ...
    def __init__(self, model: str, device: str, diarize: str):
        ...
        self.speakers = 0
        self.asr_t0: float | None = None
        self.asr_end: float | None = None  # фиксируем конец ASR, чтобы метрика скорости не «текла»
...
@click.option(
    "--batch", "fr_batch_size", type=int, default=16, show_default=True, help="Размер батча ASR"
)
@click.option(
    "--gap",
    type=float,
...
@click.option(
    "--onnx-int8", is_flag=True, help="ONNX с динамической int8-квантизацией (рычаг для CPU-only)"
)
@click.option(
    "--diar-backend",
    type=click.Choice(["torch", "onnx"]),
...
def main(
    ...
    if backend == "onnx" and dev == "mps":
        console.print(
            "[yellow]ONNX Runtime не поддерживает MPS → ASR пойдёт на CPU "
    ...
    try:
        with Live(Dashboard(progress, m), console=console, refresh_per_second=8) as live:
            ...
            if short:
                ...
                if backend == "onnx":
                    texts = infer_onnx(
                        [str(wav)], onnx_cfg, onnx_sessions, batch_size=1, progress=False
                    )
                ...
                if text and text.strip():
                    trans_segments.append(
                        TranscriptionSegment(text=text.strip(), start=0.0, end=m.audio_dur)
                    )
                ...
                if backend == "onnx":
                    ...
                    for i in range(0, len(segments), fr_batch_size):
                        ...
                        texts = infer_onnx(
                            chunk, onnx_cfg, onnx_sessions, batch_size=len(chunk), progress=False
                        )
                        for text in texts:
                            s, e = boundaries[idx]
                            idx += 1
                            if text and text.strip():
                                trans_segments.append(
                                    TranscriptionSegment(text=text.strip(), start=s, end=e)
                                )
                            m.done_segments = idx
                            m.processed = e
            ...
            if diarize == "pyannote" and trans_segments:
                ...
                trans_segments = dm.map_speakers_to_transcription(trans_segments, speaker_segments)
            ...
            result = TranscriptionResult(
                ...
                metadata={"source": str(in_path), "device": dev, "diarization": diarize},
            )
            if output:
    ...
    summary.add_row(
        "Скорость", f"{speed:.2f}× реального времени  (RTF={1/speed:.2f})" if speed else "—"
    )
    ...
    console.print(
        Panel(
            preview + ("…" if len(result.to_txt()) > 800 else ""),
            title="Превью",
            border_style="dim",
        )
    )
