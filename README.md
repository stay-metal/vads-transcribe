...
- **CLI `bloodtranscripts`** — тонкая обёртка для работы из терминала;
...
- **Слой качества** (кириллический вывод модели неприкосновенен — инвариант I1, правится только
  ...
  - самообучение глоссария: устойчивые L2-правки копятся в лог → `bloodtranscripts glossary harvest`.
...
```bash
git clone https://github.com/stay-metal/vads-transcribe.git BloodTranscripts
cd BloodTranscripts
...
bloodtranscripts transcribe meeting.m4a -d pyannote -f md -o meeting.md

# Подорожечная запись Zoom: имена спикеров = имена дорожек, диаризация не нужна
bloodtranscripts route-a "~/Zoom/2026-07-08 12.05 Дейли" -f md -o daily.md

# Пакет файлов
bloodtranscripts batch *.mp3 -o transcripts/ -d pyannote

# Машинный вывод в stdout (баннеры и прогресс идут в stderr)
bloodtranscripts transcribe a.m4a -f json > out.json
```
...
bloodtranscripts transcribe meeting.m4a -d pyannote --second-opinion -o out.txt
bloodtranscripts route-a ./запись --second-opinion -o out.md   # per-track

# Voiceprint: назвать анонимных «Спикер №N» по галерее голосов
bloodtranscripts gallery build team --track "Алиса=alice.m4a" --track "Борис=boris.m4a"
bloodtranscripts transcribe mix.m4a -d pyannote --voiceprint --gallery ~/.cache/gigaam_transcriber/galleries/team.json

# Самообучение глоссария: свернуть накопленные L2-правки в кандидатов-terms
bloodtranscripts glossary harvest            # показать
bloodtranscripts glossary harvest --apply    # дописать в config/glossary.json
```

Все флаги: `bloodtranscripts <команда> --help`.
...
export BLOODTRANSCRIPTS_PASSWORD_HASH='<хэш>' \
       BLOODTRANSCRIPTS_SESSION_KEY='<48+ случайных символов>' \
       BLOODTRANSCRIPTS_FERNET_KEY='<другие 48+ символов>' \
       BLOODTRANSCRIPTS_COOKIE_SECURE=0 BLOODTRANSCRIPTS_REQUIRE_HTTPS=0

bloodtranscripts serve --port 8000                       # api
...
Apple GPU для воркера: `export BLOODTRANSCRIPTS_DEVICE=mps`.
...
| `BLOODTRANSCRIPTS_PASSWORD_HASH` / `_USER` | Вход в web (bcrypt-хэш; user по умолчанию `admin`) |
| `BLOODTRANSCRIPTS_SESSION_KEY` / `_FERNET_KEY` | Раздельные ключи: подпись cookie / шифрование секретов в БД |
| `BLOODTRANSCRIPTS_DATA_DIR` | Каталог данных сервера (по умолчанию `~/.bloodtranscripts`) |
| `BLOODTRANSCRIPTS_DEVICE` | Устройство gpu-воркера: `cuda`/`mps`/`cpu` |
| `BLOODTRANSCRIPTS_LOCAL_WATCH_ROOT` | Корень-allowlist для локального watch |
| `BLOODTRANSCRIPTS_YANDEX_WATCH_DIR`, `YANDEX_OAUTH_CLIENT_ID/SECRET` | Интеграция с Яндекс.Диском |
| `GIGAAM_WHISPER_MODEL` / `_COMPUTE` | Модель L2 «второго мнения» (по умолчанию `small`/`int8`) |
| `GIGAAM_TRANSCRIBER_CONFIG` / `_CACHE` | Переопределение папки `config/` и кэша (`~/.cache/bloodtranscripts`) |
...
| Джобы медленные, `device_fallback: cpu` в metadata | GPU недоступен процессу (проверьте `BLOODTRANSCRIPTS_DEVICE`, для Docker — проброс GPU) |
| `bloodtranscripts serve` падает по конфигурации | Не заданы `BLOODTRANSCRIPTS_PASSWORD_HASH`/`SESSION_KEY`/`FERNET_KEY` — см. раздел «Web-сервер» |
