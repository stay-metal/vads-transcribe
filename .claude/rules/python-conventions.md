...
> Часть рулов DialogScribe. Грузится при работе с любым `.py`. Инварианты — в `CLAUDE.md`.
...
- Логируй через `logging.getLogger(__name__)` (сервер — `"dialogscribe.jobs"`), lazy `%s`-формат; пиши
